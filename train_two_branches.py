...
from cls_models.model_architectures import *
...
transform = transforms.Compose(
    [
        ...
        torchvision.transforms.Resize((int(height), int(width)), antialias=True),
    ]
)

class_0_1 = []
# root_0 = os.path.join(args.dataset_root_folder, 'CT-0')
root_0 = os.path.join(args.dataset_root_folder, 'cropped_vertebrae/Train3/image')  # 原图是NIfTI文件通常以.nii或.nii.gz为扩展名
for x in os.listdir(root_0):
    class_0_1.append(os.path.join(root_0, x))

class_0_1 = sorted(class_0_1)
class_0_1 = class_0_1[:4800]
images = class_0_1
y = [1 if 'unhealthy' in item else 0 for item in class_0_1]
class_1 = sum(y)
class_0 = len(y) - sum(y)
...
saliency_root = os.path.join(args.dataset_root_folder, 'cropped_vertebrae/Train3/saliency_maps')
...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sal_path = os.path.join(saliency_root, f'{name}')
...
train_dataset = CustomImageDataset(images=X_train, labels=y_train, process=process_scan, transform=transform,
                                   flipping=False)
val_dataset = CustomImageDataset(images=X_val, labels=y_val, process=process_scan, transform=transform, flipping=False)
test_dataset = CustomImageDataset(images=X_test, labels=y_test, process=process_scan, transform=transform,
                                  flipping=False)
...
weights = [1 - (class_0 / (class_0 + class_1)), class_0 / (class_0 + class_1)]
...
def train_iter(metric_tracker, model, x, sal, y, ids, iteration, epoch, set_name):
    ...
    targets = y.clone().detach().to(device, dtype=torch.long)
    model = model.train()
    # print(inputs.dtype)  # torch.uint8
